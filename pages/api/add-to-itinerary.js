...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!user.itineraries.includes(itineraryId)) {
    ...
    const updateResult = await db.collection('itineraries').updateOne(
      { id: itineraryId, "attractions.id": { $ne: attraction.id } }, // only if attraction id not already in array
      { $push: { attractions: attraction } }
    );

    if (updateResult.modifiedCount === 0) {
      // Check if itinerary exists
      const itineraryExists = await db.collection('itineraries').findOne({ id: itineraryId });
      if (!itineraryExists) {
